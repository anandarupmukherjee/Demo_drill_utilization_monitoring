#include "Wire.h"
//Sending data from MCU to RPi through wifi using UDP
#include <ESP8266WiFi.h>
#include <WiFiUDP.h>
#include <String.h> 
#define mySSID "UniOfCam-IoT"
#define myPASSWORD "ATAeWwwE"
...
delay(50); // Wait 5 seconds and scan again
}
 

void senddata(float gyx, float gyy, float gyz, float axx, float axy, float axz, float t)
{
    loc=WiFi.localIP().toString().c_str();
    handshake = "M;Dev: "+ loc +" A"+String(float(gyx),4)+","+String(float(gyy),4)+","+String(float(gyz),4)+","+String(float(axx),4)+","+String(float(axy),4)+","+String(float(axz),4)+","+String(float(t),2)+"C";
    Udp.beginPacket(IP, UDPPort);//Start a connection to write UDP data to the remote IP
    handshake.toCharArray(ipBuffer, 1024); 
    Udp.write(ipBuffer);
    Udp.endPacket();
    yield();
...
//if(Debug){
//Serial.print(" GyX0 = "); Serial.print(values.GyX);
//Serial.print(" | GyY0 = "); Serial.print(values.GyY);
//Serial.print(" | GyZ0 = "); Serial.print(values.GyZ);
//Serial.print(" | Tmp0 = "); Serial.print(values.Tmp);
...
senddata(values.GyX, values.GyY,values.GyZ,values.AcX, values.AcY,values.AcZ,values.Tmp);
}
